      ...
      <div className="flex gap-1 bg-white border border-[#5C4B40]/10 rounded-xl p-1">
        {[
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${activeTab === tab.key ? 'bg-[#5C4B40] text-white' : 'text-[#5C4B40]/40 hover:bg-[#5C4B40]/5'}`}
          >
            <tab.icon size={12} />
            {tab.label}
            {tab.count > 0 && (
              <span className={`px-1.5 py-0.5 rounded-full text-[8px] ${activeTab === tab.key ? 'bg-white/20' : 'bg-[#5C4B40]/10'}`}>
                {tab.count}
              </span>
            )}
          </button>
            ...
            <div className="space-y-3">
              {!isSharing && (
                <button
                  ...
                  className="w-full py-3 border-2 border-dashed border-[#5C4B40]/10 rounded-2xl text-xs font-bold text-[#5C4B40]/30 hover:text-[#5C4B40] hover:border-[#5C4B40]/30 transition-all flex items-center justify-center gap-2 disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  <Share2 size={14} />
                  Partager un projet ou une conversation
                </button>
              )}
                ...
                {isSharing && (
                  ...
                  >
                    <div className="bg-white border border-[#5C4B40]/10 rounded-2xl p-5 space-y-4">
                      <div className="text-xs font-bold text-[#5C4B40]/60 uppercase tracking-widest">Nouveau partage</div>

                      {/* Contact */}
                      <div>
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[#5C4B40]/40 block mb-2">Partager avec</label>
                        <select
                          value={shareContactId}
                          onChange={(e) => setShareContactId(e.target.value)}
                          className="w-full bg-[#F8F6F2] border border-[#5C4B40]/10 rounded-xl px-3 py-2.5 text-xs focus:outline-none"
                        >
                      ...
                      <div className="flex gap-2">
                        {[
                          ...
                          <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border text-xs font-bold transition-all ${shareType === t.key ? 'border-[#5C4B40] bg-[#5C4B40]/5' : 'border-[#5C4B40]/10'}`}
                          >
                      ...
                      <div className="flex gap-2">
                        {[
                          ...
                          <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-xl border text-[10px] font-bold uppercase tracking-widest transition-all ${sharePermission === p.key ? 'border-[#5C4B40] bg-[#5C4B40]/5' : 'border-[#5C4B40]/10 text-[#5C4B40]/40'}`}
                          >
                      ...
                      <div className="flex justify-end gap-2 pt-2">
                        <button
                          ...
                          className="px-4 py-2 text-xs font-bold text-[#5C4B40]/40"
                        >
                          Annuler
                        </button>
                        <button
                          ...
                          className="px-5 py-2 bg-[#5C4B40] text-white rounded-xl text-xs font-bold hover:scale-105 active:scale-95 transition-all disabled:opacity-50 flex items-center gap-2"
                        >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[9px] text-[#5C4B40]/30 truncate">
                          avec {contact?.name || contact?.email || share.shared_with_id} • {share.permission === 'view' ? '👁️ Lecture' : '✏️ Édition'}
                        </p>
                      </div>
                      <button
                        onClick={() => handleUnshare(share.id)}
                        className="p-1.5 text-red-300 hover:text-red-500 opacity-0 group-hover:opacity-100 rounded-lg transition-all"
