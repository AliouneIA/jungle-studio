  ...
  const handleDeploy = async () => {
    try {
      ...
      const botData = {
        ...
        model_slug: config.llmModel === 'gpt' ? 'gpt-5.2-precision' : config.llmModel === 'gemini' ? 'gemini-3-pro-preview' : config.llmModel === 'claude-opus' ? 'claude-opus-4-6' : config.llmModel === 'claude-sonnet' ? 'claude-sonnet-4-5-20250929' : 'grok-3-ultra',
        idea: config.idea,
        ...
        <div className="flex items-center gap-6">
          <div className="flex flex-col">
            ...
            <p className="text-[9px] text-[#5C4B40]/40 font-bold uppercase tracking-tight mt-1 ml-1">
              {config.llmModel === 'gpt' ? 'GPT-5.2 Precision' : config.llmModel === 'gemini' ? 'Gemini 3 Pro' : config.llmModel === 'claude-opus' ? 'Claude 4.6 Opus' : config.llmModel === 'claude-sonnet' ? 'Claude 4.5 Sonnet' : 'Grok-3 Ultra'}
            </p>
          </div>
        </div>
        <button
        ...
        <div className="w-1/2 flex flex-col bg-[#F8F6F2] relative overflow-hidden">
          ...
          <div className="flex items-center justify-between px-8 py-4 bg-transparent shrink-0">
            ...
            <div className="flex items-center gap-2 bg-white/80 p-1 px-3 rounded-lg border border-[#5C4B40]/5">
              <span className="text-[10px] font-black text-[#5C4B40]/60 uppercase tracking-tighter">{config.llmModel === 'gpt' ? 'GPT-5.2' : config.llmModel === 'gemini' ? 'Gemini 3' : config.llmModel === 'claude-opus' ? 'Claude 4.6' : config.llmModel === 'claude-sonnet' ? 'Claude 4.5' : 'Grok-3'}</span>
